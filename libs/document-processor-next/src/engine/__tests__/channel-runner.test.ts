...
import type { ChannelContract, HandlerContract, ChannelEventCheckpoint } from '../../model/index.js';
...
describe('ChannelRunner', () => {
  ...
  function createRunner(
    overrides: Partial<{
      ...
      onExecute: (handler: HandlerContract, event: BlueNode) => void;
    }> = {},
  ) {
    ...
    const isInactive = vi.fn().mockImplementation(() => overrides.isScopeInactive?.() ?? false);
    const onExecute = overrides.onExecute ?? (() => {});
    ...
    const deps: ChannelRunnerDependencies = {
      ...
      isScopeInactive: (path: string) => isInactive(),
      createContext: (
        currentScope: string,
        currentBundle: ContractBundle,
        event: BlueNode,
        allowTerminatedWork: boolean,
      ) => ({ scopePath: currentScope, event }),
      executeHandler: (handler: HandlerContract, context: { event: BlueNode }) => {
        onExecute(handler, context.event);
      },
    } satisfies Parameters<typeof ChannelRunner>[2];

    const runner = new ChannelRunner(runtime, checkpointManager, deps);
    return { runner, runtime, bundle, checkpointManager, scopePath, isInactive };
  }

  it('runs handlers and persists checkpoint for first event', () => {
    const calls: string[] = [];
    const { runner, runtime, bundle, scopePath } = createRunner({
      evaluateChannel: () => ({ matches: true, eventNode: nodeFrom({ payload: 1 }) }),
    ...
    const stored = runtime.document().get('/contracts/checkpoint/lastEvents/external');
  ...
  it('skips duplicate events based on signature', () => {
    ...
    expect(runtime.document().get('/contracts/checkpoint/lastEvents/external')).toBeInstanceOf(BlueNode);
    expect((bundle.marker(KEY_CHECKPOINT) as ChannelEventCheckpoint).lastSignatures.external).toBe('event-1');
  });
