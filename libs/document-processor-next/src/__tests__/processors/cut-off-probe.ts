...
function emitIfKind(context: Parameters<HandlerProcessor<CutOffProbe>['execute']>[1], kind?: string | null): void {
  if (!kind) return;
  const event = new BlueNode().setProperties({ kind: new BlueNode().setValue(kind) });
...
export class CutOffProbeContractProcessor implements HandlerProcessor<CutOffProbe> {
  ...
  execute(contract: CutOffProbe, context: Parameters<HandlerProcessor<CutOffProbe>['execute']>[1]): void {
