...
function shouldEmit(contract: EmitEvents, event: Parameters<HandlerProcessor<EmitEvents>['execute']>[1]['event']): boolean {
  const current = event();
...
export class EmitEventsContractProcessor implements HandlerProcessor<EmitEvents> {
  ...
  execute(contract: EmitEvents, context: Parameters<HandlerProcessor<EmitEvents>['execute']>[1]): void {
