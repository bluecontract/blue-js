...
describe('SequentialWorkflowOperationProcessor - Integration Tests', () => {
  ...
  test('processes increment operation and updates counter', async () => {
    ...
    const { state, emitted } = await blue.process(docNode, [incrementEvent]);
  ...
  test('processes decrement operation and updates counter', async () => {
    ...
    const { state, emitted } = await blue.process(docNode, [decrementEvent]);
  ...
  test('processes reset operation with specific value', async () => {
    ...
    const { state, emitted } = await blue.process(docNode, [resetEvent]);
  ...
  test('ignores operation request for non-matching operation name', async () => {
    ...
    const { state, emitted } = await blue.process(docNode, [nonMatchingEvent]);
  ...
  test('processes multiple sequential operations correctly', async () => {
    ...
    const result1 = await blue.process(docNode, [incrementEvent]);
    ...
    const result2 = await blue.process(result1.state, [decrementEvent]);
    ...
    const result3 = await blue.process(result2.state, [resetEvent]);
  ...
  test('handles complex operation with JavaScript Code step', async () => {
    ...
    const { state, emitted } = await blue.process(docNode, [complexEvent]);
