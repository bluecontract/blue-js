...
describe('Chess Game with Two Timeline Channels', () => {
  ...
  test('should play a chess game through alternating moves', async () => {
    ...
    const { state: state1 } = await blue.process(docNode, [whiteMove1]);
    ...
    const { state: state2 } = await blue.process(state1, [blackMove1]);
    ...
    const { state: state3 } = await blue.process(state2, [invalidPlayerMove]);
    ...
    const { state: state4 } = await blue.process(state3, [invalidMove]);
    ...
    const { state: state5 } = await blue.process(state4, [whiteMove2]);
    ...
    const { state: state6 } = await blue.process(state5, [blackMove2]);
    ...
    const { state: state7 } = await blue.process(state6, [whiteMove3]);
    ...
    const { state: state8 } = await blue.process(state7, [blackMove3]);
    ...
    const { state: state9 } = await blue.process(state8, [whiteMove4]);
