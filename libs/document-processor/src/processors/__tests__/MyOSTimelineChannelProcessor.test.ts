...
describe('MyOSTimelineChannelProcessor', () => {
  ...
  const createTimelineEntryEvent = (config: {
    timelineId?: string;
  } = {}) => {
    const nodeData: any = {
  ...
  describe('supports', () => {
    describe('timeline ID matching', () => {
      it('should return true for matching timeline IDs', () => {
        const channelNode = createTimelineChannelNode({ timelineId: 'timeline-123' });
        const timelineEntry = createTimelineEntryEvent({ timelineId: 'timeline-123' });
      ...
      it('should return false for non-matching timeline IDs', () => {
        const channelNode = createTimelineChannelNode({ timelineId: 'timeline-123' });
        const timelineEntry = createTimelineEntryEvent({ timelineId: 'timeline-456' });
