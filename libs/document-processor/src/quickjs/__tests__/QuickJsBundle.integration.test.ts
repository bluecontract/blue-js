...
const entryPath = path.resolve(projectRoot, 'libs/document-processor/src/quickjs/entry.ts');
  ...
  maybeIt('processes documents using bundled entry source', async () => {
    ...
    const aliasPlugin = {
      ...
      setup(buildPlugin) {
        ...
        buildPlugin.onLoad({ filter: /\.yaml$/, namespace: 'raw-yaml' }, async (args) => {
    ...
    const { processor, bridge } = await loadQuickJsBlueDocumentProcessorFromBundle({
      bundlePath,
    ...
    const initResult = await processor.initialize(documentNode, { gasBudget: 10_000 });
