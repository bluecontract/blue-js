...
| `.onChannelEvent('onIncrementEvent', 'ownerChannel', type, steps => ...)` | workflow bound to `ownerChannel`; ordinary channels keep a direct workflow-level `event` matcher, while timeline-like channels wrap the matcher under `event.message` to match the channelized timeline-entry payload | `DocBuilderChannelsDslParityTest.onChannelEventMatchesYamlDefinition` | `DocBuilder.channels.parity.test.ts`, `DocBuilder.handlers.integration.test.ts` | runtime-confirmed for timeline-like channels with underlying message-type matching |
...
| `.namedEvent('EmitReady', 'READY')` | `Conversation/Trigger Event` whose `event` node is `Common/Named Event` with required root-level `name` | same | `DocBuilder.steps.parity.test.ts`, `DocBuilder.handlers.integration.test.ts` | blank names rejected |
| `.namedEvent('EmitPayload', 'READY', payload => payload.put(...))` | same as above, plus additional root-level event fields | same | `DocBuilder.steps.parity.test.ts` | builder callback preserves insertion order |
