...
| `AITaskBuilder.expectsNamed(...)` | 2.3, 6.3 | named-event expectation node shape with root-level fields | runtime-confirmed `Common/Named Event` support |
...
| `onAIResponse(...)` named-event matcher | 2.3, 6.3 | matcher node using `Common/Named Event` shape | TypeScript adaptation uses `{ namedEvent: string }`; runtime-supported |
