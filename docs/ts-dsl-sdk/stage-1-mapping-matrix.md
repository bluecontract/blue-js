...
| `.type(typeInput)` | alias / `{blueId}` / type node / zod schema | `/type` | `DocBuilder.type(...)` | `doc-builder.general.parity.test.ts`, `type-input.test.ts` | repository-known aliases resolve to BlueIds; parity harness also registers `Custom/Type`; truly unregistered aliases are preserved inline — see deviation |
...
| `.field(path)` builder | type / description / value / required / minimum / maximum | mutate or create node at path | `FieldBuilder` | `doc-builder.sections.parity.test.ts` | constraints are written under `/constraints`; `done()` with no mutations does not create a placeholder node |
...
| operation `.request(...)` | request schema as `BlueNode` or Blue-shaped object | `/contracts/<op>/request` | `OperationBuilder.request(...)` | `doc-builder.operations.parity.test.ts` | preserves Blue-shaped `items` arrays and nested nodes without schema flattening |
