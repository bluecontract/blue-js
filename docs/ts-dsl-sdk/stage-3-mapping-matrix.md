...
|---|---|---|---|---|---|---|---|
| `myOsAdmin(...)` | 5.1, 4.3 | `DocBuilderGeneralDslParityTest.myOsAdminMatchesYamlDefinition` plus runtime scenario discovery | `DocBuilder.myOsAdmin()` / `DocBuilder.myOsAdmin(channelKey)` | `myOsAdminChannel` + `myOsAdminUpdate` + `myOsAdminUpdateImpl` | `DocBuilder.myos.parity.test.ts`, `DocBuilder.myos.integration.test.ts` | Done | Root type unchanged; runtime currently requires `request: { type: List }` on `myOsAdminUpdate` |
...
| SDPG request helper | 5.2 | `DocBuilderMyOsDslParityTest.myOsStepsMethodsProduceExpectedEventContracts` | `steps.myOs().singleDocumentPermissionGrantRequested(...)` | `MyOS/Single Document Permission Grant Requested` event | `StepsBuilder.myos.test.ts` | Done | Supports optional `requestId`; legacy `grantSessionSubscriptionOnResult` is intentionally unsupported |
